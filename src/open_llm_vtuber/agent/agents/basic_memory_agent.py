...
class BasicMemoryAgent(AgentInterface):
    ...
    def __init__(
        ...
        if self._tool_manager:
            self._formatted_tools_openai = (
                self._tool_manager.get_formatted_tools(
                    "OpenAI"
                )
            )
            self._formatted_tools_claude = (
    ...
    def set_system(self, system: str):
        ...
        logger.debug(
            f"Memory Agent: Setting system prompt: '''{system}'''"
        )
    ...
    def set_memory_from_history(
        ...
        for msg in messages:
            ...
            if isinstance(content, str) and content:
                ...
                logger.warning(
                    f"Skipping invalid message from history: {msg}"
                )
        logger.info(
            f"Loaded {len(self._memory)} messages from history."
        )
    ...
    def handle_interrupt(self, heard_response: str) -> None:
        ...
        if (
            self._memory
            and self._memory[-1]["role"] == "assistant"
        ):
        ...
        interrupt_role = (
            "system"
            if self.interrupt_method == "system"
            else "user"
        )
        self._memory.append(
            {
        ...
        logger.info(
            f"Handled interrupt with role '{interrupt_role}'."
        )
            ...
            async for event in stream:
                if event["type"] == "text_delta":
                    ...
                    if (
                        ...
                        or current_assistant_message_content[
                            -1
                        ][
                            "type"
                        ]
    ...
    async def _openai_tool_interaction_loop(
        ...
        messages = initial_messages.copy()
        current_turn_text = ""
        pending_tool_calls: Union[
            List[ToolCallObject], List[Dict[str, Any]]
        ] = []
        ...
        while True:
            if self.prompt_mode_flag:
                if self._mcp_prompt_string:
                    ...
                    logger.warning(
                        "Prompt mode active but mcp_prompt_string is empty!"
                    )
                    current_system_prompt = self._system
                tools_for_api = None
            else:
            ...
            async for event in stream:
                if self.prompt_mode_flag:
                    ...
                    if isinstance(event, str):
                        ...
                        event == "__API_NOT_SUPPORT_TOOLS__"
                    ):
                        logger.warning(
            ...
            if detected_prompt_json:
                logger.info(
                    "Processing tools detected via prompt mode JSON."
                )
                self._add_message(
                    current_turn_text, "assistant"
                )
    ...
    def _chat_function_factory(
        ...
        async def chat_with_memory(
            ...
            if self._use_mcpp and tool_mode == "Claude":
                ...
                async for (
                    output
                ) in self._claude_tool_interaction_loop(
                ...
                async for (
                    output
                ) in self._openai_tool_interaction_loop(
                    messages, tools if tools else []
                ):
                    yield output
                return
                ...
                logger.info(
                    "Starting simple chat completion."
                )
    ...
    def start_group_conversation(
        ...
        if not self._tool_prompts:
            logger.warning(
                "Tool prompts dictionary is not set."
            )
            return

        other_ais = ", ".join(
            name for name in ai_participants
        )
        ...
        if not prompt_name:
            logger.warning(
                "No group conversation prompt name found."
            )
            return

        try:
            group_context = prompt_loader.load_util(
                prompt_name
            ).format(
            ...
            logger.error(
                f"Group conversation prompt file not found: {prompt_name}"
            )
        except KeyError as e:
            logger.error(
                f"Missing formatting key in group conversation prompt: {e}"
            )
        except Exception as e:
            logger.error(
                f"Failed to load group conversation prompt: {e}"
            )
