...
class StatelessLLMWithTemplate(StatelessLLMBaseConfig):
    ...
    _OPENAI_COMPATIBLE_DESCRIPTIONS: ClassVar[
...
class OllamaConfig(OpenAICompatibleConfig):
    ...
    llm_api_key: str = Field(
        "default_api_key", alias="llm_api_key"
    )
    keep_alive: float = Field(-1, alias="keep_alive")
    unload_at_exit: bool = Field(
        True, alias="unload_at_exit"
    )
    interrupt_method: Literal["system", "user"] = Field(
...
class LmStudioConfig(OpenAICompatibleConfig):
    ...
    base_url: str = Field(
        "http://localhost:1234/v1", alias="base_url"
    )
    interrupt_method: Literal["system", "user"] = Field(
        "system", alias="interrupt_method"
    )
...
class OpenAIConfig(OpenAICompatibleConfig):
    ...
    base_url: str = Field(
        "https://api.openai.com/v1", alias="base_url"
    )
    interrupt_method: Literal["system", "user"] = Field(
        "system", alias="interrupt_method"
    )
...
class MistralConfig(OpenAICompatibleConfig):
    ...
    base_url: str = Field(
        "https://api.mistral.ai/v1", alias="base_url"
    )
    interrupt_method: Literal["system", "user"] = Field(
        "user", alias="interrupt_method"
    )
...
class DeepseekConfig(OpenAICompatibleConfig):
    ...
    base_url: str = Field(
        "https://api.deepseek.com/v1", alias="base_url"
    )
...
class GroqConfig(OpenAICompatibleConfig):
    ...
    base_url: str = Field(
        "https://api.groq.com/openai/v1", alias="base_url"
    )
    interrupt_method: Literal["system", "user"] = Field(
        "system", alias="interrupt_method"
    )
...
class ClaudeConfig(StatelessLLMBaseConfig):
    ...
    base_url: str = Field(
        "https://api.anthropic.com", alias="base_url"
    )
    llm_api_key: str = Field(..., alias="llm_api_key")
    model: str = Field(..., alias="model")
    interrupt_method: Literal["system", "user"] = Field(
    ...
    _CLAUDE_DESCRIPTIONS: ClassVar[
        dict[str, Description]
    ] = {
        "base_url": Description(
            en="Base URL for Claude API",
            zh="Claude API 的API端点",
        ),
        "llm_api_key": Description(
            en="API key for authentication",
            zh="API 认证密钥",
...
class OllamaNativeConfig(BaseModel):
    """Configuration for Ollama Native API (direct /api/chat endpoint)."""

    model: str = Field(..., alias="model")
    base_url: str = Field(
        "http://localhost:11434", alias="base_url"
    )
    ...
    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "model": Description(
            en="Name of the Ollama model to use",
            zh="要使用的 Ollama 模型名称",
        ),
        "base_url": Description(
        ...
        "temperature": Description(
            en="Sampling temperature", zh="采样温度"
        ),
        "keep_alive": Description(
            en="Keep model loaded for N seconds",
            zh="保持模型加载的秒数",
        ),
        "interrupt_method": Description(
            en="Interruption method", zh="中断方法"
        ),
    }
...
class StatelessLLMConfigs(I18nMixin, BaseModel):
    ...
    stateless_llm_with_template: (
        StatelessLLMWithTemplate | None
    ) = Field(None, alias="stateless_llm_with_template")
    openai_compatible_llm: OpenAICompatibleConfig | None = (
        Field(None, alias="openai_compatible_llm")
    )
    ollama_llm: OllamaConfig | None = Field(
        None, alias="ollama_llm"
    )
    lmstudio_llm: LmStudioConfig | None = Field(
        None, alias="lmstudio_llm"
    )
    openai_llm: OpenAIConfig | None = Field(
        None, alias="openai_llm"
    )
    gemini_llm: GeminiConfig | None = Field(
        None, alias="gemini_llm"
    )
    zhipu_llm: ZhipuConfig | None = Field(
        None, alias="zhipu_llm"
    )
    deepseek_llm: DeepseekConfig | None = Field(
        None, alias="deepseek_llm"
    )
    groq_llm: GroqConfig | None = Field(
        None, alias="groq_llm"
    )
    claude_llm: ClaudeConfig | None = Field(
        None, alias="claude_llm"
    )
    llama_cpp_llm: LlamaCppConfig | None = Field(
        None, alias="llama_cpp_llm"
    )
    mistral_llm: MistralConfig | None = Field(
        None, alias="mistral_llm"
    )
    ollama_native_llm: OllamaNativeConfig | None = Field(
        None, alias="ollama_native_llm"
    )
    ...
    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        ...
        "ollama_llm": Description(
            en="Configuration for Ollama", zh="Ollama 配置"
        ),
        ...
        "zhipu_llm": Description(
            en="Configuration for Zhipu API",
            zh="Zhipu API 配置",
        ),
        ...
        "ollama_native_llm": Description(
            en="Configuration for Ollama Native API",
            zh="Ollama 原生 API 配置",
        ),
    }
