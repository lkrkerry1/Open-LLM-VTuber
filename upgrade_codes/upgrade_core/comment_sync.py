    ...
    def sync(self) -> None:
        try:
            with open(
                ...
                default_tree: CommentedMap = self.yaml.load(
                    f
                )
            with open(
                self.user_path, "r", encoding="utf-8"
            ) as f:
            ...
            def sync_comments(
                ...
                for key in default_node:
                    if key in user_node:
                        ...
                        if hasattr(
                            default_node, "ca"
                        ) and hasattr(user_node, "ca"):
                            if key in default_node.ca.items:
                                user_node.ca.items[key] = (
                                    default_node.ca.items[
                                        key
                                    ]
                                )
                        sync_comments(
            ...
            if hasattr(default_tree, "ca") and hasattr(
                user_tree, "ca"
            ):
            ...
            self.logger.info(
                self.texts_compare["comment_sync_success"]
            )
